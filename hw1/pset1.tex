...
$\qquad \ket{-} = \tfrac{1}{\sqrt{2}} (\ket{0} - \ket{1})$ \\
...
\begin{enumerate}
    ...
    \begin{solution}\\\\
        $\braket{+ | \psi} = \frac{1}{\sqrt{2}} (\bra{0} + \bra{1})(\frac{1}{2} \ket{0} + \frac{1}{2} \ket{1} + \frac{1}{\sqrt{2}} \ket{\mathrm{vac}})$
        $ = \frac{1}{2\sqrt{2}} + \frac{1}{2\sqrt{2}} = \frac{1}{\sqrt{2}}$

        $\mathbb{P}(\ket{+}) = |\braket{+ | \psi}|^2 = \boxed{\frac{1}{2}}$
    \end{solution}
    ...
    \begin{solution}\\\\
        If $\ket{\mathrm{vac}}$ is observed, then the state collapses to $\ket{\mathrm{vac}}$.
...
\begin{enumerate}
    ...
    \begin{solution}\\\\
        ...
        $= -\frac{1}{2\sqrt{2}} \braket{1,0 | 1,+}$

        $= -\frac{1}{4} \bra{1,0} (\ket{1,0} + \ket{1,1})$
        ...
        If $\ket{\Psi^-}$ is observed, the post-measurement state will be $\ket{\Psi^-}$ because of state collapse.
    \end{solution}
    ...
    \begin{solution}\\\\
    $\ket{\psi}=\sqrt{\frac{3}{4}}\,\ket{0,0}_{AB} + \sqrt{\frac{1}{4}}\,\ket{1,+}_{AB}$
    $=\frac{\sqrt{3}}{2} \ket{0,0} + \frac{1}{2\sqrt{2}} \ket{1} \otimes (\ket{0} + \ket{1})$

    $=\frac{\sqrt{3}}{2} \ket{0,0} + \frac{1}{2\sqrt{2}} (\ket{10} + \ket{11})$

    $\mathbb{P}(A = \ket{0}) = \mathbb{P}(\ket{00}) + \mathbb{P}(\ket{01})= |\braket{00 | \psi}|^2 = \boxed{3/4}$

    If $\ket{0}$ is observed, then the post-measurement state is $\ket{00}$ because of state collapse.
    \end{solution}
\end{enumerate}

\section{Problem 4.}
Show that: $\qquad \ket{\Psi^-}=\frac{1}{\sqrt{2}}\bigl(\ket{-,+}-\ket{+,-}\bigr)$
\begin{solution}\\\\
$\frac{1}{\sqrt{2}} (\ket{-,+} - \ket{+,-})$ \\

$= \frac{1}{2\sqrt{2}} \bigl[(\ket{0} - \ket{1}) \otimes (\ket{0} + \ket{1}) - (\ket{0} + \ket{1}) \otimes (\ket{0} - \ket{1})\bigr]$ \\

$= \frac{1}{2\sqrt{2}} \bigl[\ket{0,0} + \ket{0,1} - \ket{1,0} - \ket{1,1} - \ket{0,0} + \ket{0,1} - \ket{1,0} + \ket{1,1}\bigr]$ \\

$= \frac{1}{2\sqrt{2}} \bigl(2\ket{0,1} - 2 \ket{1,0}\bigr)$ \\

$= \frac{1}{\sqrt{2}} (\ket{0,1} - \ket{1,0}) = \ket{\Psi^-}$ \qed
\end{solution}
